...
resource "aws_vpc_endpoint_route_table_association" "public_s3" {
  count = var.create_vpc && var.enable_s3_endpoint && var.enable_public_s3_endpoint && length(var.public-app_subnets) > 0 && length(var.public-network_subnets) > 0 && var.s3_endpoint_type == "Gateway" ? 1 : 0
...
resource "aws_vpc_endpoint_route_table_association" "public_dynamodb" {
  count = var.create_vpc && var.enable_dynamodb_endpoint && length(var.public-app_subnets) > 0 && length(var.public-network_subnets) > 0 && var.dynamodb_endpoint_type == "Gateway" ? 1 : 0
